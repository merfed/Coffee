...
{	
	...
	for( int i = 0; i < 256; i++ )
	{		
		int unkprops = *((unsigned int *)( File + Positions[i].Offset + 8 + 0x78 ));
		int unkeffectid = *((unsigned int *)( File + Positions[i].Offset + 8 + 0x7C ));
/*		for( int j = 0; j < nLayers; j++ )
		{
			...
			*/
		if( unkprops || unkeffectid )
		{
			printf( "\t%i: %i, %i\n", i, unkprops, unkeffectid );
			foundsomething = true;
		}
	}
