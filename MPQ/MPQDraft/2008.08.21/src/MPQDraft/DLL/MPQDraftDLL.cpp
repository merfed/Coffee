...
BOOL IsPatchTarget()
{
	...
	// Get the executable file path and compare it to the path of the patch target
	char szFileName[MAX_PATH + 1];

	DWORD dwBytesCopied = GetModuleFileName(NULL, szFileName, MAX_PATH + 1);
	QDebugWriteEntry("IsPatchTarget : GetModuleFileName returned %d", dwBytesCopied);

	BOOL bDifferentNames = _stricmp(szFileName, lpPatchContext->szTargetPath);
	QDebugWriteEntry("IsPatchTarget : _stricmp returned %d on \"%s\" and \"%s\"", 
		bDifferentNames, szFileName, lpPatchContext->szTargetPath);
