		...
		case 72:
			{
				...
				LOG("WARNING: alphaBitDepth = 72, which is recognized but not well understood. Treating as DXT5.\n");
				switch (pHeader->alphaEncoding)
				{
				case 7: 
				default:
					blpType = BLPTYPE_DXT5; squishFlags = squish::kDxt5; 
					break;
				}
				break;
			}
        ...
        for (i = 0; i < destWidth; i++) 
		{
			...
			else
			{
				...
				for (ii = 0; ii < 4; ++ii)
				{
					...
					// Not 100% sure about this but it makes sense, I think.
					deltas[ii] = int(float(deltas[ii]) * gammaWeights[ii]);
