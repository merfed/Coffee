...
void ModelEditor::SetStructure() {
	...
	events_m = (ModelEvents *) (f + header_m->ofsEvents);
...
RibbonHelper ModelEditor::getRibbons() {
	...
	for (int i = 0; i < header_m->nRibbonEmitters; i++) {
		...
		ret.nTexRefs[i]=ribbon_m[i].nTextures;
		ret.TexRefs[i]=new int[ribbon_m[i].nTextures];
		ret.TexRefs[i]=getAnimblockData(ret.TexRefs[i],ribbon_m[i].ofsTextures);

		ret.nUnkRefs[i]=ribbon_m[i].nUnknown;
		ret.unkRefs[i]=new int[ribbon_m[i].nUnknown];
		ret.unkRefs[i]=getAnimblockData(ret.unkRefs[i],ribbon_m[i].ofsUnknown);
	}
	return ret;
}
...
void ModelEditor::newRibbons(RibbonHelper help){
	...
	for (int i = 0; i < help.nRibbons; i++) {
		...
		ribbon_m[i].ofsTextures=FileSize;
		ribbon_m[i].nTextures=help.nTexRefs[i];
		writeAnimblockData(help.TexRefs[i],help.nTexRefs[i]);

		ribbon_m[i].ofsUnknown=FileSize;
		ribbon_m[i].nUnknown=help.nUnkRefs[i];
		writeAnimblockData(help.unkRefs[i],help.nUnkRefs[i]);
	}
	FillLine();
}
...
void ModelEditor::newAttachments(ModelAttachmentDef*attachments, int num) {
	...
	for (int i = 0; i < header_m->nAttachments; i++) {
		...
		attach_m[i].Enabled.nKeys = 0;
		attach_m[i].Enabled.nTimes = 0;
		attach_m[i].Enabled.ofsKeys = 0;
		attach_m[i].Enabled.ofsTimes = 0;
	}
	FillLine();
...
ModelEvents *ModelEditor::getEvents() {
	return events_m;
}
...
void ModelEditor::ResizeModel(float x,float y,float z) {
	...
	for(int i=0;i<header_m->nBoundingVertices;i++){
		boundvertices_m[i].x*= x;
		boundvertices_m[i].y*= y;
		boundvertices_m[i].z*= z;
	}
	calcVertexBox();
	calcBoundingBox();
...
void ModelEditor::ResizeModel(float factor){
	ResizeModel(factor,factor,factor);
}
