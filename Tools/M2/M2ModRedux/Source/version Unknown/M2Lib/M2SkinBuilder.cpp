...
bool M2Lib::M2SkinBuilder::CBonePartition::AddTriangle( M2Lib::CVertex* GlobalVertexList, M2Lib::CTriangle* pTriangle )
{
	...
	TriBonesSub[0] = pTriVertex->BoneIndices[0]; TotalWeight += pTriVertex->BoneWeights[0];
	TriBonesSub[1] = pTriVertex->BoneIndices[1]; TotalWeight += pTriVertex->BoneWeights[1];
	TriBonesSub[2] = pTriVertex->BoneIndices[2]; TotalWeight += pTriVertex->BoneWeights[2];
	TriBonesSub[3] = pTriVertex->BoneIndices[3]; TotalWeight += pTriVertex->BoneWeights[3];
	if ( TotalWeight != 255 )
	{
...
//UInt32 M2Lib::M2SkinBuilder::CSubMesh::CSubsetPartition::AddVertex( UInt32 VertexIndex )
//{
...
bool M2Lib::M2SkinBuilder::Build( M2Lib::M2Skin* pResult, UInt32 BoneLoD, M2Lib::M2I* pM2I, M2Lib::CVertex* pGlobalVertexList, UInt32 BoneStart )
{
	...
	for ( UInt32 i = 0; i < m_SubMeshList.size(); i++ )
	{
		for ( UInt32 j = 0; j < m_SubMeshList[i]->SubsetPartitions.size(); j++ )
		{
			...
			if ( m_SubMeshList[i]->SubsetPartitions[j]->Triangles.size() )
			{
				...
				m_SubMeshList[i]->SubsetPartitions[j]->IndexStart = IndexStart;
				m_SubMeshList[i]->SubsetPartitions[j]->IndexCount = IndexCount;
	...
	pResult->Elements[M2Lib::M2Skin::EElement_Index].SetDataSize( m_Indices.size(), m_Indices.size() * sizeof( UInt16 ), false );
	UInt16* Triangles = (UInt16*)pResult->Elements[M2Lib::M2Skin::EElement_Index].Data;
	...
	for ( UInt32 i = 0; i < m_SubMeshList.size(); i++ )
	{
		for ( UInt32 j = 0; j < m_SubMeshList[i]->SubsetPartitions.size(); j++ )
		{
			...
			if ( m_SubMeshList[i]->SubsetPartitions[j]->Triangles.size() )
			{
				...
				pSubsetOut->IndexStart = m_SubMeshList[i]->SubsetPartitions[j]->IndexStart;
				pSubsetOut->IndexCount = m_SubMeshList[i]->SubsetPartitions[j]->IndexCount;
