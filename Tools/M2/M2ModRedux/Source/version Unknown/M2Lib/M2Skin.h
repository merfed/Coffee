...
public:
	...
	CElement_SubMesh* GetSubMesh( UInt32 ID, Float32* CenterBounds, SInt32& SubMeshIndexOut );

	// gathers list of materials that affect a given sub mesh within this skin.
	void GetSubMeshMaterials( UInt32 SubMeshIndex, std::vector< CElement_Material* >& Result );

	// gathers list of flags that affect a given sub mesh within this skin.
	void GetSubMeshFlags( UInt32 SubMeshIndex, std::vector< CElement_Flags* >& Result );
