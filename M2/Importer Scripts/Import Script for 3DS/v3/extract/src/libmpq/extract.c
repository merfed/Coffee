...
int libmpq_huff_decompress(char *out_buf, int *out_length, char *in_buf, int in_length) {
	struct huffman_tree		*ht = (struct huffman_tree		*)malloc(sizeof(struct huffman_tree));
	struct huffman_input_stream	*is = (struct huffman_input_stream	*)malloc(sizeof(struct huffman_input_stream));
	struct huffman_tree_item	*hi = (struct huffman_tree_item		*)malloc(sizeof(struct huffman_tree_item));
